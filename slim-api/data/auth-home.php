<?php

$data = array(
    "user" => array(
        'id' => 1
        ,"firstname" => "Paul"
        ,"lastname" => "Schneider"
        ,"email" => "paul.schneider@yepyep.co.uk"
        ,"accessKey" => "b21def8e-dcd1-11e3-b9e0-e2cb53404599"
        ,"loggedIn" => date('d M Y H:i:s')
    )
    ,"channels" => array(
        0 => array(
            "id" => 1
            ,"name" => "Channel 1"
            ,"sefName" => "channel-1"
            ,"colour" => "#0000ff"
            ,"icon" => "full/path/to/file/"
            ,"subChannels" => array(
                0 => array(
                    "id" => 6
                    ,"name" => "Sub Channel 1"
                    ,"sefName" => "sub-channel-1"
                    ,'categories' => array(
                        0 => array(
                            "id" => 1
                            ,"name" => "Category 1"
                            ,"sefName" => "category-1"
                        )
                        ,1 => array(
                            "id" => 2
                            ,"name" => "Category 2"
                            ,"sefName" => "category-2"
                        )
                        ,2 => array(
                            "id" => 3
                            ,"name" => "Category 3"
                            ,"sefName" => "category-3"
                        )
                    )
                )
                ,1 => array(
                    "id" => 7
                    ,"name" => "Sub Channel 2"
                    ,"sefName" => "sub-channel-2"
                    ,'categories' => array(
                        0 => array(
                            "id" => 6
                            ,"name" => "Category 1"
                            ,"sefName" => "category-1"
                        )
                        ,1 => array(
                            "id" => 7
                            ,"name" => "Category 2"
                            ,"sefName" => "category-2"
                        )
                        ,2 => array(
                            "id" => 8
                            ,"name" => "Category 3"
                            ,"sefName" => "category-3"
                        )
                        ,3 => array(
                            "id" => 9
                            ,"name" => "Category 4"
                            ,"sefName" => "category-4"
                        )
                        ,4 => array(
                            "id" => 10
                            ,"name" => "Category 4"
                            ,"sefName" => "category-4"
                        )
                    )
                )
                ,2 => array(
                    "id" => 8
                    ,"name" => "Sub Channel 3"
                    ,"sefName" => "sub-channel-3"
                    ,'categories' => array(
                        0 => array(
                            "id" => 11
                            ,"name" => "Category 1"
                            ,"sefName" => "category-1"
                        )
                        ,1 => array(
                            "id" => 12
                            ,"name" => "Category 2"
                            ,"sefName" => "category-2"
                        )
                        ,2 => array(
                            "id" => 13
                            ,"name" => "Category 3"
                            ,"sefName" => "category-3"
                        )
                        ,3 => array(
                            "id" => 14
                            ,"name" => "Category 4"
                            ,"sefName" => "category-4"
                        )
                        ,4 => array(
                            "id" => 15
                            ,"name" => "Category 4"
                            ,"sefName" => "category-4"
                        )
                    )
                )
            )
        )
        ,1 => array(
            "id" => 2
            ,"name" => "Channel 2"
            ,"sefName" => "channel-2"
            ,"colour" => "#0000ff"
            ,"icon" => "full/path/to/file/"
            ,"subChannels" => array(
                0 => array(
                    "id" => 9
                    ,"name" => "Sub Channel 1"
                    ,"sefName" => "sub-channel-1"
                    ,'categories' => array(
                        0 => array(
                            "id" => 16
                            ,"name" => "Category 1"
                            ,"sefName" => "category-1"
                        )
                        ,1 => array(
                            "id" => 17
                            ,"name" => "Category 2"
                            ,"sefName" => "category-2"
                        )
                    )
                )
                ,1 => array(
                    "id" => 10
                    ,"name" => "Sub Channel 2"
                    ,"sefName" => "sub-channel-2"
                    ,'categories' => array(
                        0 => array(
                            "id" => 21
                            ,"name" => "Category 1"
                            ,"sefName" => "category-1"
                        )
                        ,1 => array(
                            "id" => 22
                            ,"name" => "Category 2"
                            ,"sefName" => "category-2"
                        )
                        ,2 => array(
                            "id" => 23
                            ,"name" => "Category 3"
                            ,"sefName" => "category-3"
                        )
                    )
                )
                ,2 => array(
                    "id" => 11
                    ,"name" => "Sub Channel 3"
                    ,"sefName" => "sub-channel-3"
                    ,'categories' => array(
                        0 => array(
                            "id" => 26
                            ,"name" => "Category 1"
                            ,"sefName" => "category-1"
                        )
                        ,1 => array(
                            "id" => 27
                            ,"name" => "Category 2"
                            ,"sefName" => "category-2"
                        )
                        ,2 => array(
                            "id" => 28
                            ,"name" => "Category 3"
                            ,"sefName" => "category-3"
                        )
                        ,3 => array(
                            "id" => 29
                            ,"name" => "Category 4"
                            ,"sefName" => "category-4"
                        )
                        ,4 => array(
                            "id" => 30
                            ,"name" => "Category 4"
                            ,"sefName" => "category-4"
                        )
                    )
                )
            )
        )
        ,2 => array(
            "id" => 3
            ,"name" => "Channel 3"
            ,"sefName" => "channel-3"
            ,"colour" => "#0000ff"
            ,"icon" => "full/path/to/file/"
            ,"subChannels" => array(
                0 => array(
                    "id" => 12
                    ,"name" => "Sub Channel 1"
                    ,"sefName" => "sub-channel-1"
                )
                ,1 => array(
                    "id" => 13
                    ,"name" => "Sub Channel 2"
                    ,"sefName" => "sub-channel-2"
                )
                ,2 => array(
                    "id" => 14
                    ,"name" => "Sub Channel 3"
                    ,"sefName" => "sub-channel-3"
                )
            )
        )
        ,3 => array(
            "id" => 5
            ,"name" => "Channel 5"
            ,"sefName" => "channel-5"
            ,"colour" => "#0000ff"
            ,"icon" => "full/path/to/file/"
            ,"subChannels" => array(
                0 => array(
                    "id" => 18
                    ,"name" => "Sub Channel 1"
                    ,"sefName" => "sub-channel-1"
                )
                ,1 => array(
                    "id" => 19
                    ,"name" => "Sub Channel 2"
                    ,"sefName" => "sub-channel-2"
                )
                ,2 => array(
                    "id" => 20
                    ,"name" => "Sub Channel 3"
                    ,"sefName" => "sub-channel-3"
                )
            )
        )
    )
    ,"sponsors" => array(
        0 => array(
            "id" => 1
            ,"title" => "A Test Sponsor (advert)"
            ,"url" => "a/link/to/the/sponsor/site/or/product"
            ,"media" => array(
                "url" => "link/to/the/product/image"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,1 => array(
            "id" => 1
            ,"title" => "A Second Test Sponsor (advert)"
            ,"url" => "a/link/to/the/sponsor/site/or/product"
            ,"media" => array(
                "url" => "link/to/the/product/image"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
    )
    ,"articles" => array(
        0 => array(
            "title" => "Article 1 title"
            ,"sefName" => "article-1-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,1 => array(
            "title" => "Article 2 title"
            ,"sefName" => "article-2-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,2 => array(
            "title" => "Article 3 title"
            ,"sefName" => "article-3-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,3 => array(
            "title" => "Article 4 title"
            ,"sefName" => "article-4-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,4 => array(
            "title" => "Article 5 title"
            ,"sefName" => "article-5-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
    )
    ,"picks" => array(
        0 => array(
            "title" => "Article 6 title"
            ,"sefName" => "article-6-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "single"
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,1 => array(
            "title" => "Article 7 title"
            ,"sefName" => "article-7-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "double"
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,2 => array(
            "title" => "Article 8 title"
            ,"sefName" => "article-8-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "double"
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,3 => array(
            "title" => "Article 8 title"
            ,"sefName" => "article-8-title"
            ,"displayAs" => "single"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,4 => array(
            "title" => "Article 9 title"
            ,"sefName" => "article-9-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "single"
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,5 => array(
            "title" => "Article 10 title"
            ,"sefName" => "article-10-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "double"
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,6 => array(
            "title" => "Article 11 title"
            ,"sefName" => "article-11-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "single"
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,7 => array(
            "title" => "Article 12 title"
            ,"sefName" => "article-12-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "single"
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,8 => array(
            "title" => "Article 13 title"
            ,"sefName" => "article-13-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "double"
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,9 => array(
            "title" => "Article 14 title"
            ,"sefName" => "article-15-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "single"
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
    )
    ,"whatsOn" => array(
        0 => array(
            "title" => "Monday Article Title"
            ,"sefName" => "monday-article-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "double"
            ,"venue" => array(
                "id" => 1
                ,"title" => "Venue Name One"
                ,"sefName" => "venue-name-one"
                ,"showDate" => "19 May 2014"
                ,"showTime" => "19:30"
                ,"showPrice" => 24.99
            )
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,1 => array(
            "title" => "Tuesday Article Title"
            ,"sefName" => "tuesday-article-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "single"
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,2 => array(
            "title" => "Wednesday Article Title"
            ,"sefName" => "wednesday-article-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "single"
            ,"venue" => array(
                "id" => 1
                ,"title" => "Venue Name Two"
                ,"sefName" => "venue-name-two"
                ,"showDate" => "24 May 2014"
                ,"showTime" => "18:30"
                ,"showPrice" => 45.99
            )
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,3 => array(
            "title" => "Thursday Article Title"
            ,"sefName" => "thursday-article-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "double"
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,4 => array(
            "title" => "Friday Article Title"
            ,"sefName" => "friday-article-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "single"
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,5 => array(
            "title" => "Saturday Article Title"
            ,"sefName" => "saturday-article-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "double"
            ,"venue" => array(
                "id" => 1
                ,"title" => "Venue Name Three"
                ,"sefName" => "venue-name-three"
                ,"showDate" => "09 June 2014"
                ,"showTime" => "20:00"
                ,"showPrice" => 9.99
            )
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
        ,6 => array(
            "title" => "Sunday Article Title"
            ,"sefName" => "sunday-article-title"
            ,"intro" => "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
            ,"displayAs" => "single"
            ,"media" => array(
                "url" => "link/to/the/article/thumbnail"
                ,"alt" => "Alternative text to be displayed if the image can\'t be found"
                ,"title" => "Title text displayed when the image is hovered over"
            )
        )
    )
);
